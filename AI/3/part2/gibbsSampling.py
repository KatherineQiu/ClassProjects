...
def gibbsAsk(X, e, bn, N):
    ...
    # local variables:
    vectorN = {True: 0, False: 0}   # a vector of counts for each value of X, initially zero
    vectorZ = []    # the non-evidence variables in bn
    for key in bn:
        if bn[key] not in e:
            vectorZ.append(key)
    vectorx = e.copy()  # the current state of the network, initially copied from e

    ran = random.random()
    if ran <= 0.5:
        vectorx.update({vectorZ[0]: True})
    else:
        vectorx.update({vectorZ[0]: False})

    for j in range(1, int(N)+1):
        for nonevar in vectorZ: # non-evidence variable
            # set the value of Zi in vectorx by sampling from P(Zi|mb(Zi))
